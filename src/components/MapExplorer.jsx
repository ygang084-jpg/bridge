 ...
 *
 * 지도는 카카오맵(`KakaoMap`)이 그린다. 키가 없거나 SDK 가 뜨지 않으면 격자 배경 +
 * 상대 위치 마커로 되돌린다 — 이 화면이 원래 쓰던 방식이다. 그때 마커 위치는 실제
 * 위경도를 캔버스에 선형 투영한 것이라 서로의 상대 위치만 진짜다. 절대 위치나 축척은
 * 지도가 아니므로 화면에 그렇게 적는다.
 *
 ...
 *   · 확대/축소·내 위치 버튼 → 카카오 지도의 끌기·휠 확대를 그대로 쓴다. 격자
 *     배경으로 되돌아갔을 때는 조작할 지도가 없어 버튼만 남으므로 두지 않았다
