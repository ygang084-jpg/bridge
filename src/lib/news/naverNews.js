 ...
 * 기본 검색어.
 *
 * '교량'이 가장 넓게 걸리고, 나머지는 관리 이력과 직접 관련된 보도를 놓치지
 * 않으려고 함께 던진다. 같은 기사가 여러 검색어에 걸려도 url 로 중복이 제거되므로
 * 겹치는 검색어를 두는 것이 손해가 아니다.
 *
 * 쓰지 않는 낱말과 이유 :
 *   · '다리' — 신체 부위와 겹쳐 관계없는 기사가 대량으로 섞인다.
 *   · '대교' — 고유명사 교량(성산대교 등)을 넓게 잡지만, 이름에 '대교'가 들어간
 *     회사·학교 기사가 함께 들어온다. 같은 종류의 문제다.
 *
 * 검색어를 늘리면 호출 수가 비례해 늘어난다. 네이버 검색 API 한도는 하루
 * 25,000회이고 지금은 검색어 × 1회/일이라 여유가 크다.
 *
